...
| [0019](0019-tab2-input-contract-workbook-per-recording.md) | Tab 2 input contract — one xlsx workbook per recording (`trace`/`spikes`/optional `metadata` sheets); whole untrimmed recording on absolute time, regions defined in-app (disjoint, overlap = hard error) and bracketed at analysis time; spike-sufficiency reported not gated; CSV (two files) as field fallback; replaces ADR-0016 layout (0016 stays Accepted); SheetJS becomes v1 critical-path | Proposed |
